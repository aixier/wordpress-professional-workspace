...
const Header = () => {
  ...
  const txt = lang != "zh" ? "TubeScanner - Social Media Account Analysis Assistant" : "TubeScanner - 跨境社媒账号分析助手"
  const label1 = lang != "zh" ? "Features" : "特性"
  const label2 = lang != "zh" ? "Product Features" : "特性"
  const label3 = lang != "zh" ? "Features" : "特性"
  const label4 = lang != "zh" ? "Features" : "特性"
  ...
  return (
    <header className="py-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <nav className="relative z-50 flex justify-between items-center">
        ...
        <div className="flex items-center md:gap-x-12 flex-1">
          ...
          >
            <Image
              ...
              height={32}
            />
            <span className="text-gray-950 dark:text-gray-300 hidden md:block whitespace-nowrap">
              {/* {siteConfig.name} */}
              {txt}
            </span>
          </Link>
        ...
        {/* <ul className="hidden md:flex items-center justify-center gap-6 flex-1 ml-[50px]">
          {links.map((link) => (
            <li key={link.label}>
              <Link
